...
class MultiPartHandler(nodeParams: NodeParams, register: ActorRef, db: IncomingPaymentsDb) extends ReceiveHandler {
  ...
  case class PaymentRequestResult(replyTo: ActorRef, res: Try[IncomingPayment])
  ...
  override def handle(implicit ctx: ActorContext, log: DiagnosticLoggingAdapter): Receive = {
    case ReceivePayment(amount_opt, desc, expirySeconds_opt, extraHops, fallbackAddress_opt, paymentPreimage_opt, paymentType) =>
      ...
      Future {
        ...
        IncomingPayment(paymentRequest, paymentPreimage, paymentType, paymentRequest.timestamp.seconds.toMillis, IncomingPaymentStatus.Pending)
      }.onComplete(res => ctx.self ! PaymentRequestResult(replyTo, res))

    case PaymentRequestResult(replyTo, res) =>
      res match {
        case Success(incomingPayment) =>
          incomingPayments = incomingPayments + (incomingPayment.paymentRequest.paymentHash -> incomingPayment)
          //db.addIncomingPayment(paymentRequest, paymentPreimage, paymentType)
          replyTo ! incomingPayment.paymentRequest
