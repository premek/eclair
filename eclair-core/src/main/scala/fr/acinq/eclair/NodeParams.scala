  ...
  def makeNodeParams(config: Config, instanceId: UUID, nodeKeyManager: NodeKeyManager, channelKeyManager: ChannelKeyManager,
    ...
    NodeParams(
      ...
      routerConf = RouterConf(
        ...
        searchHeuristicsRiskFactor = config.getDouble("router.path-finding.heuristics.risk-factor"),
        searchHeuristicsMaxEdgeProbability = config.getDouble("router.path-finding.heuristics.max-edge-probability"),
        searchHeuristicsFailurePenaltyBaseMsat = MilliSatoshi(config.getLong("router.path-finding.heuristics.failure-penalty-base-msat")),
        searchHeuristicsFailurePenaltyProportionalMillionths = config.getLong("router.path-finding.heuristics.failure-penalty-proportional-millionths"),
