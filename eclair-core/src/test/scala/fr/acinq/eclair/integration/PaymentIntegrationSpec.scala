...
class PaymentIntegrationSpec extends IntegrationSpec {
  ...
  test("send an HTLC A->B->G->C using heuristics to select the route") {
    ...
    sender.send(nodes("A").paymentInitiator, SendPaymentRequest(amountMsat, pr.paymentHash, nodes("C").nodeParams.nodeId, maxAttempts = 1, fallbackFinalExpiryDelta = finalCltvExpiryDelta, routeParams = integrationTestRouteParams.map(_.copy(ratios = Some(WeightRatios(0, 0, 0, 1))))))
    sender.expectMsgType[UUID]
